...
//  Modified for SY STORE - VIP Lock Screen Edition 🛡️.
//
...
class StoreAuthManager: ObservableObject {
                ...
                if let json = try JSONSerialization.jsonObject(with: data) as? [String: Any],
                   let status = json["status"] as? String {
                    ...
                    if status == "suspended" {
                        ...
                        // الكود سليم! نحفظه في الجهاز ونسمح بالدخول
                        UserDefaults.standard.set(cleanCode, forKey: "activation_code")
                        
                        // إذا كان الكود جديداً (valid)، نقوم بحرقه وتحويله إلى مستخدم (used)
                        if status == "valid" {
                            self.markCodeAsUsed(code: cleanCode)
                        }
    ...
    private func markCodeAsUsed(code: String) {
        ...
        let body: [String: Any] = [
            "status": "used",
            "usedDate": ISO8601DateFormatter().string(from: Date()),
            "udid": deviceID
        ]
        ...
        ZStack {
            ...
            VStack(spacing: 30) {
                ...
                Button("شراء كود تفعيل؟") {
                    if let url = URL(string: "https://t.me/your_telegram_channel") {
...
struct SYStoreApp: App {
        ...
        WindowGroup {
            ZStack {
                if authManager.isChecking {
                    // شاشة التحميل أثناء فحص السيرفر
                    VStack {
                        ProgressView("جاري فحص الاشتراك...")
                    }
                    .frame(maxWidth: .infinity, maxHeight: .infinity)
                    .background(Color(UIColor.systemBackground))
                } else if authManager.isAuthorized {
                    // 🟢 فتح المتجر بالكامل إذا كان الكود سليم 🟢
                    VStack {
                    ...
                    .onReceive(NotificationCenter.default.publisher(for: .heartbeatInvalidHost)) { _ in
                        DispatchQueue.main.async {
                            UIAlertController.showAlertWithOk(
                                ...
                                message: "ملف الربط الخاص بك غير متوافق مع هذا الجهاز."
                            )
                        }
                    }
                    ...
                    // 🔴 شاشة القفل إذا كان الكود غير موجود أو تم تجميده 🔴
                    ActivationView()
                }
            }
            .onAppear {
    ...
    // دالة التوقيع التلقائي الصامتة (تعمل بأمان لأن الواجهة محظورة أصلاً لغير المشتركين)
    ...
    private func _handleURL(_ url: URL) {
        // [نفس الكود الأصلي لم يتغير لتجنب أي مشاكل في الاستيراد]
